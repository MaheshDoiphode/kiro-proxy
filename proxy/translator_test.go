...
func TestParseModelThinkingAndEffort(t *testing.T) {
	tests := []struct {
		...
		{"no thinking", "claude-sonnet-4.6", "claude-sonnet-4.6", false, ""},
		{"default thinking", "claude-sonnet-4.6-thinking", "claude-sonnet-4.6", true, ""},
		{"thinking low", "claude-sonnet-4.6-thinking-low", "claude-sonnet-4.6", true, "low"},
		{"thinking medium", "claude-sonnet-4.6-thinking-medium", "claude-sonnet-4.6", true, "medium"},
		{"thinking high", "claude-sonnet-4.6-thinking-high", "claude-sonnet-4.6", true, "high"},
		{"thinking xhigh", "claude-sonnet-4.6-thinking-xhigh", "claude-sonnet-4.6", true, "xhigh"},
		{"custom mapping thinking high", "vendor/custom-mini-thinking-high", "claude-haiku-4.5", true, "high"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotModel, gotThinking, gotEffort := ParseModelThinkingAndEffort(tt.input, "-thinking")
			if gotModel != tt.wantModel || gotThinking != tt.wantThinking || gotEffort != tt.wantEffort {
				t.Fatalf("ParseModelThinkingAndEffort(%q) = (%q, %v, %q), want (%q, %v, %q)",
					tt.input, gotModel, gotThinking, gotEffort, tt.wantModel, tt.wantThinking, tt.wantEffort)
			}
		})
	}
}
