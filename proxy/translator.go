...
func ParseModelThinkingAndEffort(model string, thinkingSuffix string) (string, bool, string) {
	lower := strings.ToLower(model)
	thinking := false
	effort := ""

	suffixLower := strings.ToLower(thinkingSuffix)
	for _, eff := range []string{"low", "medium", "high", "xhigh"} {
		fullSuffix := suffixLower + "-" + eff
		if strings.HasSuffix(lower, fullSuffix) {
			thinking = true
			effort = eff
			model = model[:len(model)-len(fullSuffix)]
			lower = strings.ToLower(model)
			break
		}
	}

	if !thinking && strings.HasSuffix(lower, suffixLower) {
		thinking = true
	...
	for _, m := range config.GetModelMappings() {
		...
		if key != "" && value != "" && strings.Contains(lower, key) {
			return value, thinking, effort
		}
	}

	if matches := claudeVersionPattern.FindStringSubmatch(lower); matches != nil {
		return matches[1] + "." + matches[2], thinking, effort
	}
...
func ParseModelAndThinking(model string, thinkingSuffix string) (string, bool) {
	m, t, _ := ParseModelThinkingAndEffort(model, thinkingSuffix)
	return m, t
}
