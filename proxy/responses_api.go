...
func (h *Handler) handleOpenAIResponses(w http.ResponseWriter, r *http.Request) {
	...
	actualModel, thinking, effort := ParseModelThinkingAndEffort(prepared.OpenAIRequest.Model, thinkingCfg.Suffix)
	prepared.OpenAIRequest.Model = actualModel
	if prepared.OpenAIRequest.ReasoningEffort != "" {
		effort = prepared.OpenAIRequest.ReasoningEffort
	}
	thinking = resolveThinkingWithEffort(thinking, effort)
	...
	h.applyReasoningEffort(kiroPayload, prepared.OpenAIRequest.Model, effort)
